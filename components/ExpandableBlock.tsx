      ...
      <div className="flex flex-col items-center justify-between overflow-clip h-full relative z-10">
        <div className={`flex grow ${isExpanded ? 'items-start pt-[20px]' : 'items-center'} justify-between px-10 w-full h-full`}>
          ...
          <button 
            ...
            className="flex items-center justify-center px-[10px] md:px-[0] border border-white rounded-[3px] size-[50px] hover:bg-white hover:bg-opacity-10 transition-all duration-300"
          >
            {isExpanded ? (
              <div className="bg-[#d9d9d9] h-0.5 mx-[2px] w-[20px]" />
            ) : (
              <PlusIcon />
            )}
          </button>
